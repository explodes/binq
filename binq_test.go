...
func TestFile_Put_Overwrite_InPlace(t *testing.T) {
	...
	must(t, bq.Put(key, smallValue))

	err := bq.Put(key, smallerValue)
...
func TestFile_PutGet_NotFound(t *testing.T) {
	...
	must(t, bq.Put(key1, value1))
	must(t, bq.Put(key2, value2))
